...
class Showcase
{
	...
	public static function install($product_slug, $adminPageId='', $api_url='')
	{
		if( is_admin() && !defined('DOING_AJAX') )
		{
			$me = new Showcase($product_slug, $adminPageId, $api_url);
		...
		if( $isCurrent )
		{
			...
			if( !empty($posts) )
			{
				...
				for($i=0 ; $i<count($posts) ; ++$i )
				{
					if( $posts[$i] )
					{
						$page['tabs'][self::TAB_ID]['groups'][] = array(
							'id' => "_$i",
							'title' => $posts[$i]->title,
	...
	protected function __construct($slug, $adminPageId='', $url='')
	{
